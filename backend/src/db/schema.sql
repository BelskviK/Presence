...
-- users.office_location_id references geofence_locations, so it's added here
-- (after that table exists) rather than in the users table definition above.
...
create table if not exists attendance (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
  -- Deliberately no unique(user_id, date): a user can clock in/out more than
  -- once per day (e.g. left and came back), so a day can have multiple rows.
);
